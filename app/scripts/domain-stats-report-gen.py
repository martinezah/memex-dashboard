...
outputFile = '/var/local/cdr-reports/domain_stats_report.csv'
domain = ''
if len(sys.argv) > 1:
    outputFile = sys.argv[1]

log.info("Connect to ES")
es = Elasticsearch("els.istresearch.com",
                   http_auth=('memex', '3vYAZ8bSztbxmznvhD4C'),
                   use_ssl=True, verify_certs=False, timeout=10000)
...
if len(sys.argv) > 2:
    ...
    all_domains = es.search(index="memex-domains", body=query)

log.debug(json.dumps(all_domains, indent=4))

#catQuery = Dict()
#catQuery.aggs.by_domain.terms.field = "url.domain"
#catQuery.aggs.by_domain.terms.size = 0
#catQuery.aggs.by_domain.aggs.by_type.terms.field = "_type"
#catQuery.aggs.by_domain.aggs.by_type.terms.size = 0
#catQuery.aggs.by_domain.aggs.by_type.aggs.by_category.terms.field = "url.category"
#catQuery.aggs.by_domain.aggs.by_type.aggs.by_category.terms.size = 0
#all_categories = es.search(index="memex-domains", body=catQuery)
...
for item in all_domains['aggregations']['by_domain']['buckets']:
    for typed_item in item['by_type']['buckets']:
        ...
        csv_file.writerow({'domain': typed_item['key'],
                           ...
                           'sections_scraping': '', #typed_item['sections_scraping']['key'],
